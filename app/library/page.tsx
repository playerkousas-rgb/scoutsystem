...
export default function LibraryPage() {
  ...
  const bookmarks = (data?.libraryBookmarks || [])
  ...
  const branchTags = Array.from(new Set((data?.libraryBookmarks || []).flatMap(b => b.branchTags))).filter(Boolean);
  ...
  return <div className="stack">
    ...
    <section className="grid">
      <div className="card"><span className="badge blue">本旅收藏</span><h2>{data?.libraryBookmarks.length || 0}</h2><p className="muted">領袖標記後才會出現在這裡</p></div>
      <div className="card"><span className="badge gold">待處理 / 跟進</span><h2>{(data?.libraryBookmarks || []).filter(b => ['new','following'].includes(b.status)).length}</h2><p className="muted">需要領袖判斷是否轉活動</p></div>
      <div className="card"><span className="badge green">已發布 / 已轉活動</span><h2>{(data?.libraryBookmarks || []).filter(b => ['converted','published'].includes(b.status)).length}</h2><p className="muted">已進入本旅流程</p></div>
    </section>
    ...
    <section className="card stack">
      <h2>本旅已標記通告</h2>
      {!bookmarks.length && <p className="muted">暫時沒有符合條件的標記通告。</p>}
      {bookmarks.map(b => <div key={b.id} className="card" style={{ boxShadow: 'none' }}>
        ...
        {b.notes && <p>{b.notes}</p>}
        <p className="muted">負責人：{data?.users.find(u => u.id === b.ownerUserId)?.name || '-'} {data?.users.find(u => u.id === b.ownerUserId)?.role ? `（${roleLabel[data.users.find(u => u.id === b.ownerUserId)!.role]}）` : ''}</p>
      </div>)}
    </section>
