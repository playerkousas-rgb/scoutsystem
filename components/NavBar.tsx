'use client';
import Link from 'next/link';

export default function NavBar() {
  return (
    <nav className="flex items-center justify-between bg-[#001f3f] text-white px-8 py-2.5 shadow-xl sticky top-0 z-50">
      <div className="flex items-center space-x-10">
        <Link href="/" className="font-black text-2xl tracking-tighter">🏰 ScoutSystem</Link>
        <div className="flex items-center space-x-6 text-[11px] font-bold uppercase tracking-widest opacity-80">
          <Link href="/activities" className="hover:text-blue-400">📅 行事曆</Link>
          <Link href="/market" className="hover:text-blue-400">🏪 外掛市集</Link>
          <Link href="/setup" className="hover:text-blue-400">⚙️ 旅團接入</Link>
          <Link href="/download" className="hover:text-blue-400">⬇️ 下載</Link>
          <Link href="/update" className="hover:text-blue-400">📢 更新</Link>
        </div>
      </div>
      <Link href="/login" className="bg-blue-600 hover:bg-blue-500 px-6 py-1.5 rounded-full text-[11px] font-black transition-all">登入系統</Link>
    </nav>
  );
}
